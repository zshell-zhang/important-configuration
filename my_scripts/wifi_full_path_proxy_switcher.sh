...
if [[ ${command}x = 'onx' ]]; then
    ...
    sudo iptables -t nat -D PREROUTING -d 127.0.0.0/24 -j RETURN
    sudo iptables -t nat -D PREROUTING -d 192.168.0.0/16 -j RETURN
    sudo iptables -t nat -D PREROUTING -d 10.42.0.0/16 -j RETURN
    sudo iptables -t nat -D PREROUTING -p tcp -s 10.42.0.0/16 -j REDIRECT --to-ports 12345
